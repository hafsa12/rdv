...
class GestionRendezVous extends AtexoPage {
	...
	public function onLoad()
	{
		...
		if(!$this->isPostBack) {
			...
			$ressource = Atexo_User_CurrentUser::isRessource();
			...
			if($ressource){
                ...
                $idEtablissement = $tAgent->getIdEtablissementAttache();
                $this->listeEtablissement->setSelectedValue($idEtablissement);
                $this->listeEtablissement->Enabled = false;
            }
	...
	public function loadRessource() {
		...
		$this->listeRessource->SelectedValue = Atexo_User_CurrentUser::getIdAgent();
		$this->listeRessource->Enabled = false;
	}
	...
	protected function suggestNames() {
		...
		if(Atexo_User_CurrentUser::isRessource()){
            $criteriaVo->setIdRessource(Atexo_User_CurrentUser::getIdAgent());
        }
		//$criteriaVo->setIdEtablissementAttache(Atexo_User_CurrentUser::getIdEtablissementGere());
	...
	private function envoyerSms($rdv)
	{
		...
		$message = str_replace('[TEL]', $rdv->getTEtablissement()->getTelephoneRdv(),$message);
...
//		$etiq->writeString(0, 3,$profil, $hdr_format);
        $etiq->writeString(0, 0,$codeRdv, $hdr_format);
		$etiq->writeString(0, 2,$dateRdv, $hdr_format);
		$etiq->writeString(0, 3,$heureDebut, $hdr_format);
...
//		$etiq->writeString(0, 8,$etab, $hdr_format);
		...
		$etiq->writeString(0, 7,$etat, $hdr_format);
		
//		$etiq->writeString(0, 13,$annuleLe, $hdr_format);
//		$etiq->writeString(0, 14,$annulePar, $hdr_format);

		$etiq->writeString(0, 1,$nomClient, $hdr_format);
	/*	$etiq->writeString(0, 12,$dateNaissance, $hdr_format);
		$etiq->writeString(0, 13,$emailClient, $hdr_format);
		$etiq->writeString(0, 14,$phoneClient, $hdr_format);*/
		$etiq->writeString(0, 8,$champSupp1, $hdr_format);
		$etiq->writeString(0, 9,$champSupp2, $hdr_format);
		$etiq->writeString(0, 10,$champSupp3, $hdr_format);
		$c=1;
		...
		//		case Prado::localize('CITOYEN') : $etiq->write($c,2,/*$data["NOM"]." ".$data["PRENOM"]*/Prado::localize('CITOYEN'),$corps_format);
		//		break;
		...
		//	$etiq->writeString($c,3,$data["PROFIL_UTILISATEUR"],$corps_format);
            $etiq->writeString($c,0,$data["CODE_RDV"],$corps_format);
			$etiq->writeString($c,2,$data["DATE_DEB_RDV"],$corps_format);
			$etiq->writeString($c,3,$data["TIME_DEB_RDV"],$corps_format);
	...
	//		$etiq->writeString($c,8,$data["DENOMINATION_ETABLISSEMENT_ATTACHE"],$corps_format);
			...
			$etiq->writeString($c,7,$data["ETAT_RDV"],$corps_format);
	//		$etiq->writeString($c,13,$data["DATE_ANNULATION_FR"],$corps_format);
	//		$etiq->writeString($c,14,$data["ANNULE_PAR"],$corps_format);
			$etiq->writeString($c,1,$data["NOM"]." ".$data["PRENOM"],$corps_format);
			/*$etiq->writeString($c,12,$data["DATE_NAISSANCE"],$corps_format);
			$etiq->writeString($c,13,$data["MAIL"],$corps_format);
			$etiq->writeString($c,14,$data["TELEPHONE"],$corps_format);*/
			$etiq->writeString($c,8,$data["TEXT1"], $corps_format);
			$etiq->writeString($c,9,$data["TEXT2"], $corps_format);
			$etiq->writeString($c,10,$data["TEXT3"], $corps_format);
			$c++;
		}
...
if($prestation->getIdPrestation()===1636) $path = "ressources/word/recap_rdv_52_ar"  . ".docx";
		...
		try {
			...
			if($rdv->getTCitoyen()->getText1()){
				$templateProcessor->setValue('libelle_rc','</w:t><w:br/><w:t>'.'RC :');
				$templateProcessor->setValue('num_rc',$rdv->getTCitoyen()->getText1().'</w:t><w:br/><w:t>');
			}
			else{
			     $templateProcessor->setValue('libelle_rc','');
			     $templateProcessor->setValue('num_rc','');
			}
